...
export default function EmployeeList() {
    ...
    return (
        <PersonaLearnLayout>
            <header
                className=" h-20 flex items-center justify-between px-8 bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-200 p-8">
                <div>
              ...
              {employeesData.stats.totalChange}{' '}
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
                    ...
                    <div className="flex items-end justify-between">
                        ...
                        <span
                            className="text-blue-500/60 text-xs font-medium">{employeesData.stats.averageAIScoreLabel}</span>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tr className="border-b border-slate-100">
                            <th className="px-8 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest">Имя
                                сотрудника
                            </th>
                            <th className="px-6 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest">Должность</th>
                            <th className="px-6 py-5 text-xs font-bold text-slate-400 uppercase tracking-widest">Прогресс
                                обучения
                            </th>
                            ...
                            <tr key={employee.id} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-8 py-5">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`size-10 rounded-full ${getAvatarColor(employee.avatarColor)} flex items-center justify-center font-bold text-sm`}>
                                            {employee.avatar}
                                        </div>
          ...
          <span className="text-xs font-medium text-slate-500">
            Показано {employeesData.employees.length} из {employeesData.stats.totalEmployees} сотрудников
                    ...
                    <div className="flex gap-1">
                        <button
                            className="size-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:bg-white">
                            <Icon name="chevron_left" className="text-lg"/>
                        </button>
                        <button
                            className="size-8 flex items-center justify-center rounded-lg bg-blue-500 text-white text-xs font-bold">1
                        </button>
                        <button
                            className="size-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 text-xs font-bold hover:bg-white">
                            2
                        </button>
                        <button
                            className="size-8 flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 text-xs font-bold hover:bg-white">
